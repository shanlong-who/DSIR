...
*Code chunks below are not evaluated when this vignette is built, 
since they make network calls. Run them in your own R session to 
see the output.*
...
`theme_dsi()` is a `ggplot2` theme tuned for WHO-style charts — clean 
panels, a modest grid, and a consistent accent colour. Use it as a 
drop-in replacement for `theme_minimal()` whenever a chart is heading 
into a WHO deliverable.
